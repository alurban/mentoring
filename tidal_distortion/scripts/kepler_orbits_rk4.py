...
def rk4(y, h, rhs, extraArgs):
    """ Performs numerical integration using the RK4 method on an equation
        with previous value y, step size h and right-hand side rhs. """
    k1 = rhs(y, *extraArgs)
    k2 = rhs(y + h*k1/2, *extraArgs)
    k3 = rhs(y + h*k2/2, *extraArgs)
    k4 = rhs(y + h*k3, *extraArgs)
    # Note: we are returning only the integral part to make sure we can handle
    # sign flips at the turning points.
...
for i in xrange(len(E)):
    for j in xrange(1, len(t)):
        ...
        rhs_a = -rk4(a[i][j-1], dt, dadt, (E[i], L))
        ...
        a_proposed = a[i][j-1] + rhs_a
        if -G*M**2/a_proposed + (L/a_proposed)**2/M > E[i]:
            rhs_a *= -1

        # carry on with the simulation
        a[i].append( a[i][j-1] + rhs_a )
        phi[i].append( phi[i][j-1] + rk4(a[i][j-1], dt, dphidt, (L,)) )
...
for i in xrange(len(E)):
    ax1.plot(t[::50]/Porb, np.array(a[i][::50])/1000, c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax1.fill_between(t[::50]/Porb, 0, 11, facecolor='Tomato', edgecolor='Tomato', alpha=0.5)
ax1.fill_between(t[::50]/Porb, 0, 12, facecolor='Tomato', edgecolor='Tomato', alpha=0.5)
ax1.fill_between(t[::50]/Porb, 0, 13, facecolor='Tomato', edgecolor='Tomato', alpha=0.5)
...
for i in xrange(len(E)):
    ax2.plot(t[::50]/Porb, np.rad2deg(np.array(phi[i][::50])), c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax2.set_xlim([0, 5])
ax2.set_ylim([0, 1800])
...
for i in xrange(len(E)):
    ax3.plot(t[::50]/Porb, np.array(v[i][::50])/299792458., c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax3.set_xlim([0, 5])
...
for i in xrange(1, len(E)):
    ...
    ax1.plot(np.array(a[i][::50])/1000, energy[::50]/1e45, c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
...
for i in xrange(len(E)):
    energy = np.array([(M/4)*dadt(x, E[i], L)**2 - G*M**2/x + (L/x)**2/M for x in a[i]])
    ax2.plot(t[::50]/Porb, energy[::50]/1e45, c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax2.set_xlim([0, 5])
ax2.set_ylim([-15, 5])
...
for i in xrange(len(E)):
    ...
    ax3.plot(t[::50]/Porb, ang[::50]/1e42, c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax3.set_xlim([0, 5])
ax3.set_xlabel(r'$t/P_{\rm circular}$ (unitless)')
ax3.xaxis.set_major_formatter(ticker.FormatStrFormatter("%.2g"))
ax3.set_ylim([0, 10])
...
for i in xrange(len(E)-1):
    ax.plot(phi[i][::50], np.array(a[i][::50])/1000, c[i], linewidth=2., label='$E =$ %s$E_L$' % frac[i])
ax.set_rmax(80)
