...
m1, m2 = 36*MSun, 29*MSun  # GW150914 component masses in kg
mu, M = m1*m2/(m1+m2), m1 + m2  # reduced mass, total mass in kg
# the observed merger frequency of GW150914 was ~250 Hz
rISCO = (G*M / (pi*250)**2)**(1./3)  # ISCO radius, in meters
...
# the observed lower frequency of GW150914 was ~35 Hz.
r0 = (G*M / (pi*35)**2)**(1./3)  # initial separation, in meters
Tmerge = time_to_merger(r0)  # expected time until merger
fISCO = 250  # GW150914 observed merger frequency (Hz)
...
ax4.plot([t.min(), 1.05*t.max()], [0]*2, 'k--', linewidth=1.)
ax4.annotate('number of cycles = %.3g' % (phi[-1]/pi), xy=(0.05, 1.8),
     xycoords='data', size=10, ha="center", va="center")
rmu, rM = 2*G*mu/c**2, 2*G*M/c**2
D = 410 * 3.086e22  # 410 Mpc (measured GW150914 distance), in meters
...
ax4.plot(t, h/1e-21, 'DarkSlateGray')
ax4.set_xlim([t.min(), 1.05*t.max()])
ax4.set_xlabel('time (sec)')
ax4.xaxis.set_major_formatter(ticker.FormatStrFormatter("%.3g"))
ax4.set_ylim([-1.05*h.max()/1e-21, 1.05*h.max()/1e-21])
ax4.set_ylabel(r'strain (10$^{-21}$)')
ax4.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.3g"))

# Save the figure.
fig.tight_layout()
plt.savefig('inspiral_orbit_parameters_BBH.pdf')
...
ax2.set_ylim([1, 1.5*max(LGW)/ESun])
ax2.set_yscale('log')
ax2.set_ylabel(r'luminosity ($M_{\rm \odot}c^2$/s)')
ax2.yaxis.set_major_formatter(ticker.FormatStrFormatter("%.4g"))

# Save the figure.
fig.tight_layout()
plt.savefig('inspiral_energies_BBH.pdf')
...
ax.plot(phi, np.array(r)/1000, 'DodgerBlue', linewidth=2.)
ax.set_rmax(1000)
ax.set_rticks(np.array([100, 1000]))
ax.grid(True)
ax.set_xticklabels(['0$^{\circ}$', '45$^{\circ}$', '', '135$^{\circ}$', '180$^{\circ}$',
    '225$^{\circ}$', '270$^{\circ}$', '315$^{\circ}$'])
ax.yaxis.set_major_formatter(ticker.FormatStrFormatter("%d"))
ax.set_title( 'number of orbits = %.2g' % (phi[-1]/(2*pi)) )

# Save the figure.
fig.tight_layout()
plt.savefig('inspiral_diagram_BBH.pdf')
...
try:
    ...
    star1 = ax.scatter(x1/2000, y1/2000, c='Black', s=2000, edgecolors='none')
    star2 = ax.scatter(-x1/2000, -y1/2000, c='Black', s=2000, edgecolors='none')
    ax.grid(False)
    ax.set_xlim([-500, 500])
    ...
    inset_ax.set_ylim([-1.05*h.max()/1e-21, 1.05*h.max()/1e-21])
    inset_ax.set_ylabel('strain (10$^{-21}$)')
